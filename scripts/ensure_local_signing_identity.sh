...
# A stable, local-only signing identity keeps the app's designated requirement
# unchanged across development rebuilds, so macOS TCC grants remain valid.
IDENTITY_NAME="${SNACK_MEET_SIGNING_IDENTITY_NAME:-Snack Meet Local Code Signing}"
SIGNING_DIR="${SNACK_MEET_SIGNING_DIR:-$HOME/Library/Application Support/Snack Meet/Signing}"
KEYCHAIN_PATH="$SIGNING_DIR/snack-meet.keychain-db"
...
unlock_managed_keychain_if_present() {
  ...
  if [[ ! -f "$PASSWORD_FILE" ]]; then
    print -u2 "Snack Meet signing keychain exists but its password file is missing: $PASSWORD_FILE"
    exit 1
  fi
  local password
...
[[ -n "$OPENSSL_BIN" ]] || {
  print -u2 "OpenSSL is required to create the local Snack Meet signing identity."
  exit 1
}

print -u2 "Creating a stable local code-signing identity for Snack Meet..."
umask 077
mkdir -p "$SIGNING_DIR"
...
temporary_directory="$(mktemp -d -t snack-meet-signing)"
trap 'rm -rf "$temporary_directory"' EXIT
certificate="$temporary_directory/snack-meet.cer"
private_key="$temporary_directory/snack-meet.key"
archive="$temporary_directory/snack-meet.p12"
archive_password="$($OPENSSL_BIN rand -hex 24)"

$OPENSSL_BIN req -x509 -newkey rsa:2048 -sha256 -days 3650 -nodes \
  -subj "/CN=$IDENTITY_NAME/O=Snack Meet Local" \
...
identity="$(find_identity)"
if [[ -z "$identity" ]]; then
  print -u2 "Unable to create a valid Snack Meet signing identity."
  exit 1
fi
