...
fn main() {
    ...
    // Prepare FFmpeg from repository/local trusted input. This never downloads
    // executables from the former upstream project.
...
fn detect_and_report_gpu_capabilities() {
    ...
    println!("cargo:warning=🚀 Building Snack Meet for: {}", target_os);
