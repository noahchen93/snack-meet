    ...
    match result {
        Some((provider, model)) => {
                ...
                || (provider_type == "parakeet" && provider == "parakeet")
            {
                ...
                // Fall back to the settings table (whisperModel) before the built-in default,
                // so the model configured in the UI is honored by external imports.
                if provider_type == "whisper" {
                    if let Some(whisper_model) = configured_whisper_model(app).await? {
                        return Ok(whisper_model);
                    }
                }
...
/// Read `whisperModel` from the `settings` table if present.
async fn configured_whisper_model<R: Runtime>(app: &AppHandle<R>) -> Result<Option<String>> {
    let Some(app_state) = app.try_state::<AppState>() else {
        return Ok(None);
    };
    let whisper_model: Option<String> =
        sqlx::query_scalar("SELECT whisperModel FROM settings LIMIT 1")
            .fetch_optional(app_state.db_manager.pool())
            .await
            .map_err(|e| anyhow!("Failed to query whisperModel: {}", e))?;
    let whisper_model = whisper_model.filter(|m| !m.trim().is_empty());
    if whisper_model.is_some() {
        log::info!("Using whisperModel from settings: {:?}", whisper_model);
    }
    Ok(whisper_model)
}
