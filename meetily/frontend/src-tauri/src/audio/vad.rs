...
impl ContinuousVadProcessor {
    ...
    pub fn process_audio(&mut self, samples: &[f32]) -> Result<Vec<SpeechSegment>> {
        ...
        let resampled_audio = if self.sample_rate == 16000 {
            ...
            // CRITICAL FIX: Use the high-quality anti-aliased sinc resampler used by
            // the retranscription path. The previous custom linear-interpolation
            // resampler introduced aliasing that caused Silero VAD to miss ~99% of
            // speech in real meeting audio.
            let mut resampled =
                super::audio_processing::resample_audio(samples, self.sample_rate, 16000);
            // Clamp any Gibbs-overshoots; Silero rejects samples outside [-1, 1].
