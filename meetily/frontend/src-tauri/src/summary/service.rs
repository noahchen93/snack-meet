...
impl SummaryService {
    ...
    async fn rename_meeting_folder(
        ...
        let Some(folder_path) = folder_path.filter(|p| !p.trim().is_empty()) else {
            return Ok(());
        };
        let old_path = std::path::PathBuf::from(&folder_path);
        if !old_path.is_dir() {
            info!(
                "Meeting folder not found on disk, skipping rename: {}",
                folder_path
            );
            return Ok(());
        }
        ...
        // Native recordings store their real start in metadata.created_at. Older
        // Snack Record imports encode it in meeting_name, so retain that fallback.
        // In either case duration_seconds produces the actual end time.
        let (start_ts, end_ts): (String, String) = (|| -> Option<(String, String)> {
            ...
            let dur =
                chrono::Duration::from_std(std::time::Duration::from_secs_f64(duration)).ok()?;
            let end = start + dur;
            let fmt = "%Y-%m-%d_%H-%M";
        ...
        info!(
            ...
            old_name, new_folder_name, meeting_id
        );
        Ok(())
    }
            ...
            Ok((final_markdown, english_markdown, num_chunks)) => {
                    ...
                    extract_meeting_name_from_markdown(&final_markdown).filter(|n| !n.is_empty())
                {
                        ...
                        MeetingsRepository::update_meeting_name(&pool, &meeting_id, &name).await
                    {
                        ...
                        if let Err(e) = Self::rename_meeting_folder(&pool, &meeting_id, &name).await
                        {
                            error!("Failed to rename meeting folder for {}: {}", meeting_id, e);
                        }
                    }
                }
