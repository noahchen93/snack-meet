...
async fn trigger_microphone_permission() -> Result<bool, String> {
    audio::permissions::ensure_microphone_permission().await
}
...
pub fn run() {
    ...
    #[cfg(any(target_os = "macos", windows, target_os = "linux"))]
    {
        builder = builder.plugin(tauri_plugin_single_instance::init(|app, args, cwd| {
            ...
            crate::external_trigger::trigger_import_from_args(app, &args);
    ...
    builder
        ...
        .setup(|_app| {
            ...
            // Handle external import args on fresh launch
            // (`open -b com.meetily.ai --args --import <path> --title <title>`)
            let launch_args: Vec<String> = std::env::args().collect();
            if launch_args.iter().any(|a| a == "--import") {
                let app_handle = _app.handle().clone();
                tauri::async_runtime::spawn(async move {
                    crate::external_trigger::trigger_import_from_args(&app_handle, &launch_args);
                });
            }
